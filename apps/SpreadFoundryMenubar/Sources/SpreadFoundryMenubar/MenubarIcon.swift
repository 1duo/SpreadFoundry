...
enum MenubarIcon {
    static func statusImage(status: String) -> NSImage {
        let image = draw(size: CGSize(width: 28, height: 28), status: status, template: true)
    ...
    private static func draw(size: CGSize, status: String, template: Bool) -> NSImage {
        ...
        arc.move(to: CGPoint(x: 11 * scale, y: 13 * scale + yOffset))
        arc.curve(
            to: CGPoint(x: 33 * scale, y: 32 * scale + yOffset),
            controlPoint1: CGPoint(x: 14 * scale, y: 27 * scale + yOffset),
            controlPoint2: CGPoint(x: 23 * scale, y: 33 * scale + yOffset)
        )
        arc.stroke()
        ...
        wing.move(to: CGPoint(x: 13 * scale, y: 28 * scale + yOffset))
        wing.line(to: CGPoint(x: 28 * scale, y: 28 * scale + yOffset))
        wing.line(to: CGPoint(x: 34 * scale, y: 34 * scale + yOffset))
        wing.stroke()

        ink.setFill()
        let lowerBar = CGRect(x: 16 * scale, y: 16 * scale + yOffset, width: 13 * scale, height: 3.8 * scale)
